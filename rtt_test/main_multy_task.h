...
void plic_init(void)
{
    ...
    PLIC_sourceX_cfg(0x0,0x1,0x1,0x1);
    PLIC_sourceX_cfg(0x0,0x2,0x1,0x1);
    PLIC_sourceX_cfg(0x0,0x3,0x2,0x1);
    PLIC_sourceX_cfg(0x0,0x4,0x3,0x1);
	PLIC_sourceX_cfg(0x0,0x5,0x4,0x1);
    PLIC_sourceX_cfg(0x0,0x6,0x5,0x1);
    PLIC_sourceX_cfg(0x0,0x7,0x6,0x1);
    PLIC_sourceX_cfg(0x0,0x8,0x7,0x1);
	PLIC_sourceX_cfg(0x0,0x9,0x7,0x1);
...
uint32_t plic_irq_base_mie;
uint32_t plic_irq_base_mepc;
uint32_t plic_irq_base_thr;
...
{
	uint32_t cid = PLIC_get_claim();
	switch( cid ) {
		case 0:	plic_irq_vec_handler_0(); break;
		case 1:	plic_irq_vec_handler_1(); break;
		case 2:	plic_irq_vec_handler_2(); break;
		case 3:	plic_irq_vec_handler_3(); break;
		case 4:	plic_irq_vec_handler_4(); break;
		case 5:	plic_irq_vec_handler_5(); break;
		case 6:	plic_irq_vec_handler_6(); break;
		case 7:	plic_irq_vec_handler_7(); break;
		case 8:	plic_irq_vec_handler_8(); break;
		case 9:	plic_irq_vec_handler_9(); break;
		default: break;
	}
}
void irq_test_delay( int i )
{
	test_delay( 1 );
}


void __attribute__((interrupt)) plic_irq_vec_handler_1_fast(void)
{
	putchar( '{' );
	putchar( '!' );
	putchar( '}' );

	reg32(PLIC_BASE + PLIC_CLAIM_COMPLETED) = 1;
}

//-------------------------------------------------

uint32_t plic_irq0_mie;
uint32_t plic_irq0_mepc;
uint32_t plic_irq0_thr;
void plic_irq_vec_handler_0(void)
{
	plic_irq0_mie = read_csr(mie);
	plic_irq0_mepc = read_csr(mepc);
	plic_irq0_thr = PLIC_get_threshold();
	PLIC_set_threshold( 0 );
	...
	close_ie();
	reg32(PLIC_BASE + PLIC_CLAIM_COMPLETED) = 0;
	PLIC_set_threshold( plic_irq0_thr );
	write_csr( mepc, plic_irq0_mepc );
	write_csr( mie, plic_irq0_mie );
	set_mmode();
}
uint32_t plic_irq1_mie;
uint32_t plic_irq1_mepc;
uint32_t plic_irq1_thr;
void plic_irq_vec_handler_1(void)
{
	plic_irq1_mie = read_csr(mie);
	plic_irq1_mepc = read_csr(mepc);
	plic_irq1_thr = PLIC_get_threshold();
	PLIC_set_threshold( 0 );
	...
	reg32(PLIC_BASE + PLIC_CLAIM_COMPLETED) = 1;
	PLIC_set_threshold( plic_irq1_thr );
...
void plic_irq_vec_handler_2(void)
{
	...
	PLIC_set_threshold( plic_irq2_thr );
...
uint32_t plic_irq6_thr;
void plic_irq_vec_handler_6(void)
{
	plic_irq6_mie = read_csr(mie);
	plic_irq6_mepc = read_csr(mepc);
	plic_irq6_thr = PLIC_get_threshold();
	...
	putchar( ')' );
	reg32(PLIC_BASE + PLIC_CLAIM_COMPLETED) = 6;
	PLIC_set_threshold( plic_irq6_thr );
...
uint32_t plic_irq7_thr;
void plic_irq_vec_handler_7(void)
{
	plic_irq7_mie = read_csr(mie);
	plic_irq7_mepc = read_csr(mepc);
	plic_irq7_thr = PLIC_get_threshold();
	PLIC_set_threshold( 6 );
	...
	putchar( ')' );
	reg32(PLIC_BASE + PLIC_CLAIM_COMPLETED) = 7;
	PLIC_set_threshold( plic_irq7_thr );
...
uint32_t plic_irq8_thr;
void plic_irq_vec_handler_8(void)
{
	plic_irq8_mie = read_csr(mie);
	plic_irq8_mepc = read_csr(mepc);
	plic_irq8_thr = PLIC_get_threshold();
	PLIC_set_threshold( 7 );
	...
	putchar( '>' );
	reg32(PLIC_BASE + PLIC_CLAIM_COMPLETED) = 8;
	PLIC_set_threshold( plic_irq8_thr );
...
uint32_t plic_irq9_thr;
void plic_irq_vec_handler_9(void)
{
	plic_irq9_mie = read_csr(mie);
	plic_irq9_mepc = read_csr(mepc);
	plic_irq9_thr = PLIC_get_threshold();
	PLIC_set_threshold( 7 );
	...
	putchar( '>' );
	reg32(PLIC_BASE + PLIC_CLAIM_COMPLETED) = 9;
	PLIC_set_threshold( plic_irq9_thr );
...
int main()
{
    ...
    rt_thread_init(&thread1_pcb, "big", thread1, RT_NULL, &thread1_stack[0], sizeof(thread1_stack), 2, 1);
    rt_thread_startup(&thread1_pcb);

    rt_thread_init(&thread2_pcb, "sort", thread2, RT_NULL, &thread2_stack[0], sizeof(thread2_stack), 2, 1);
    rt_thread_startup(&thread2_pcb);

    rt_thread_init(&thread3_pcb, "path", thread3, RT_NULL, &thread3_stack[0], sizeof(thread3_stack), 2, 1);
    rt_thread_startup(&thread3_pcb);

    rt_thread_init(&thread4_pcb, "maxs", thread4, RT_NULL, &thread4_stack[0], sizeof(thread4_stack), 2, 1);
    rt_thread_startup(&thread4_pcb);

    rt_thread_init(&thread5_pcb, "filter", thread5, RT_NULL, &thread5_stack[0], sizeof(thread5_stack), 2, 1);
    rt_thread_startup(&thread5_pcb);

    rt_thread_init(&thread6_pcb, "find", thread6, RT_NULL, &thread6_stack[0], sizeof(thread6_stack), 2, 1);
    rt_thread_startup(&thread6_pcb);

    rt_thread_init(&thread7_pcb, "float", thread7, RT_NULL, &thread7_stack[0], sizeof(thread7_stack), 2, 1);
    rt_thread_startup(&thread7_pcb);

	#ifdef CONFIG_RV
	rvv_init();

	rt_thread_init(&thread8_pcb, "vector8", thread8, RT_NULL, &thread8_stack[0], sizeof(thread8_stack), 2, 1);
    rt_thread_startup(&thread8_pcb);
	
	rt_thread_init(&thread9_pcb, "vector9", thread9, RT_NULL, &thread9_stack[0], sizeof(thread9_stack), 2, 1);
